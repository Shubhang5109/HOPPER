...
Ask HOPPER always retrieves matching records **locally first** — a
rule-based keyword parser (`js/ask.js`) that recognizes host, pathogen-type,
source-database, region, and data-domain signals in a plain-language
question and turns them into the same structured filter object the Search
page uses. On top of that, there are three ways the question actually gets
answered, in priority order:

1. **Bring-your-own-key (works immediately, no deployment):** on the Ask
   HOPPER page, anyone viewing the site can open "Set up real AI answers"
   and paste their own Anthropic API key. HOPPER then calls Claude
   **directly from that browser**, using Anthropic's documented
   `anthropic-dangerous-direct-browser-access` header — no server, no
   Worker, nothing to deploy. The key is sent only to `api.anthropic.com`
   and is never written anywhere except (optionally, if the person checks
   "remember on this device") that browser's own `localStorage`.
2. **Shared proxy (optional, for every visitor without their own key):**
   if `js/config.js`'s `ASK_HOPPER_PROXY_URL` is set, HOPPER calls that
   instead — see [`docs/ai-proxy-setup.md`](docs/ai-proxy-setup.md) for
   deploying the small Cloudflare Worker in
   `cloudflare-worker/ask-hopper-proxy.js`.
3. **Local rule-based mode (the default, zero configuration):** if neither
   of the above is set up, HOPPER answers from the same local retrieval
   step directly — no external call is made, and the public GitHub Pages
   deployment works with nothing configured at all.

In every mode, if the AI step fails (bad key, rate limit, network issue,
proxy down), Ask HOPPER automatically falls back to the local rule-based
answer with a visible explanation — it never just breaks.
...
`Source_URL`, a `Provenance` note, and a `Last_Checked` date. Where a
citation could be independently verified, the source's record page also
links its reference publication (title, authors, year, and a working
DOI/URL) — see `data/sources.json`'s `Reference_Publication` field and the
Databases page. See the site's **About → Data & provenance** section.
...
│   ├── config.js                optional Ask HOPPER AI proxy URL (empty = local-only)
...
├── cloudflare-worker/
│   └── ask-hopper-proxy.js     optional serverless proxy for real-LLM Ask HOPPER answers
├── docs/
│   ├── data-model.md
│   ├── interoperability.md
│   ├── ai-proxy-setup.md       how to deploy the optional AI proxy (no local install needed)
